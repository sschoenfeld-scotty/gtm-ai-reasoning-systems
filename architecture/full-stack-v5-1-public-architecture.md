...
**Status**  
Current canonical Full Stack architecture. Functional as a reasoning system and matched private Operating Manual and Execution Prompt. Full Stack v5.1 preserves the v5 reasoning spine and adds Compressed Diagnosis as an additive reasoning capability. The capability is field-test architecture and should not be described as formally validated.
...
flowchart LR
    ...
    B --> CD{Compressed Diagnosis\nwhen eligible}
    CD --> C[Competing Explanations]
...
Full Stack v5.1 preserves the governing v5 reasoning spine and adds one conditional capability.

### Compressed Diagnosis

Compressed Diagnosis asks whether additional diagnostic work is likely to improve the decision enough to justify the time, delay, opportunity cost, or other consequence required to obtain more evidence.

The capability does not permit prescription without diagnosis.

It recognizes that repeated, materially analogous, outcome-tested experience or other accumulated evidence can create a strong prior that narrows the search space and reduces the amount of additional current-state investigation a decision requires.

The public governing principles are

> **Experience can compress diagnosis without bypassing it.**

> **Learning may transfer across contexts. Diagnosis does not.**

Reasoning depth and diagnostic expansion remain separate. Deep reasoning can conclude that additional evidence collection has low decision value. Compressed Diagnosis does not justify shallow reasoning when novelty, contradiction, irreversibility, asymmetric downside, path dependency, or other material uncertainty warrants deeper scrutiny.

A strong prior remains revisable. Material contradictory evidence can defeat it. Familiarity, confidence, seniority, or repeated exposure alone do not establish that the current case matches the prior pattern.

When a bounded and reversible action can produce useful discriminating evidence more efficiently than waiting, the intervention can function as a diagnostic probe. The response then re-enters the reasoning system as new evidence.

## v5 Refinements Retained in v5.1

Six later observations exposed narrower gaps inside the accepted v5 architecture. They remain v5 refinements carried forward into v5.1 because they deepen existing functions rather than changing Full Stack's purpose or inserting another peer stage.
...
The detailed gate, compression criteria, and clarification mechanics remain private.
...
v5.1 also makes evidence sufficiency decision-relative. Diagnosis requires sufficient evidence rather than a fixed volume of current-state evidence. When accumulated evidence creates a strong prior, Full Stack asks whether additional information is likely to improve the decision enough to justify the cost or delay required to obtain it.

Experienced operating judgment may inform that prior when the analogy is materially similar and has been tested by later outcomes. It does not become verified current fact merely because the pattern is familiar.
...
When a diagnosis was compressed, remaining uncertainty is carried into Prognosis rather than hidden. The system asks what becomes more likely if the provisional diagnosis is correct and what material consequence follows if it is wrong.
...
When action is being considered under a compressed or provisional diagnosis, Strategic Adjudication can also compare the risk of acting under remaining uncertainty with the cost or opportunity loss of waiting for additional evidence. Difficult-to-reverse or asymmetric downside raises the evidence burden. Bounded, reversible action can support staged learning when strategically justified.
...
When a bounded and reversible intervention can produce discriminating evidence more efficiently than waiting, it can also function as a diagnostic probe. The probe is defined by what response would strengthen the diagnosis, weaken it, favor an alternative, or require the action to stop.
...
When the action was selected under Compressed Diagnosis, the response can strengthen the prior, weaken it, expose another constraint, or show that fuller diagnostic expansion is now required. A favorable artifact, intervention, adoption signal, or execution result does not by itself prove the reasoning that produced it.
...
Full Stack v5.1 is not an AI model, an alignment technique, an agent runtime, a risk-management methodology, or a capital-allocation framework.
...
The public [Full Stack v5.1 Evaluation Plan](../evaluation/full-stack-v5-1-evaluation-plan.md) defines the next evidence step for testing Compressed Diagnosis while continuing to inspect the inherited v5 capabilities for regression. Existing v4 and v5 evidence remains evidence for the versions that actually produced it and is not silently relabeled as v5.1 validation.
...
Full Stack v5 remains preserved as historical canonical source material and as a public architecture artifact in this repository. v4 remains an earlier historical version.

v5.1 preserves the governing v5 reasoning spine and adds Compressed Diagnosis as an additive capability because it introduces a recurring decision the prior architecture did not explicitly govern. The system can now reason about whether additional diagnostic expansion has enough expected decision value to justify its cost or delay.

Reasoning Depth Routing, Reflexivity, Cascade Integrity, Decision Trace Integrity, Anchoring Resistance, and Behavioral Inference Discipline remain v5 refinements carried forward into v5.1. The Mandatory Reasoning Depth Selection, Routing Framing Guardrail, and Context Intake Discipline remain nested refinements rather than new peer stages.

Future minor wording, examples, evaluation changes, execution clarifications, and nested refinements should update v5.1 without creating another numbered release. Another point release should require another additive reasoning capability that materially expands what Full Stack must reason about while preserving the governing reasoning spine. A future major version should require a meaningful change in purpose, governing architecture, or reasoning spine.
